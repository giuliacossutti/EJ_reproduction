...
root -l EJ_reproduction/tasks/20250428_Task17/MergeHighMjj.C 
*/
...
void DrawHistos2D(std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,Double_t lumi,TString XTitle,TString YTitle,TString ZTitle,std::vector<std::array<Double_t,4>> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  for(Int_t i = 0; i < hist.size(); ++i){
    ...
    outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250428_Task17/%s_sample%d.pdf",outname,i);
    c->SaveAs(outpdf);
...
void MergeHighMjj()
{
  ...
  TString outname;
  outname.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250428_Task17/histos_highmjj.root");
