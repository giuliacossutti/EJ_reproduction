...
root -l EJ_reproduction/tasks/20250331_Task15/MergeHistos.C'(sel)' 
...
// File vector must have 2 signal files first and then all bkg files
...
void DrawHistos(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,std::vector<Color_t> color,TString XTitle,TString YTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  // Customize signal histos
  CustomizeHist(hist.at(0), kFullCircle, color.at(0));
  CustomizeHist(hist.at(1), kFullSquare, color.at(1));
  ...
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250331_Task15/%s_%d.pdf",outname,sel);
  c->SaveAs(outpdf);
...
void DrawHistosStack(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,std::vector<Color_t> color,TString XTitle,TString YTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250324_Task14/%s_%d_stack.pdf",outname,sel);
  c->SaveAs(outpdf);
...
void DrawHistos2D(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,TString XTitle,TString YTitle,TString ZTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  for(Int_t i = 0; i < hist.size(); ++i){
    ...
    outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250324_Task14/%s_sample%d_sel%d.pdf",outname,i,sel);
    c->SaveAs(outpdf);
...
void MergeHistos(Int_t sel=0)
{
  ...
  TString signame_ddbar;
  signame_ddbar.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250331_Task15/signal_ddbar_%d.root",sel);
  ...
  TFile *f_ddbar = new TFile(signame_ddbar);
  ...
  std::vector<TFile*> infiles = {f_sig,f_ddbar,f_bkgT,f_bkgH};
  std::vector<Color_t> colors = {kBlack,kMagenta,kGreen,kRed};
  std::vector<TString> legentry = {TString::Format("f #bar{f} -> Zv, #sigma = %.1f pb",xsec_sig),TString::Format("f #bar{f} -> Zv -> d #bar{d}, #sigma = %.1f pb",xsec_sig),TString::Format("top production, #sigma = %.1f pb",xsec_bkgT),TString::Format("2->2 hard QCD jets, #sigma = %.0f pb",xsec_bkgH)};
  ...
  // Calculate weights
  std::vector<Double_t> xsec = {xsec_sig,xsec_sig,xsec_bkgT,xsec_bkgH};
  std::vector<Double_t> ntot = {ntot_sig,ntot_sig,ntot_bkgT,ntot_bkgH};
...
/*
  ...
  TString outname;
  outname.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250331_Task15/histos_sel%d.root",sel);

  TFile *f = new TFile(outname,"RECREATE");

  // 1D histograms

  std::array<Double_t,4> leglimitsInvmass = {0.516291,0.564236,0.827068,0.873264};
...
*/
  std::array<Double_t,4> leglimitsNC = { 0.443609,0.534722,0.754386,0.831597};
