...
void ExamplePVtxFind(const char* inputFile, Int_t Nevent = 5)
{
	...
	TClonesArray* branchTrack = treeReader->UseBranch("TrackSmearing");
	...
	for (Int_t entry = 0; entry < Nev; ++entry)
	{
		...
		for (Int_t n = 0; n < NtrG; n++) {
			...
			std::cout << "Parameter vector: " <<(*PrSk[0])[0]<<"  "<<(*PrSk[0])[1]<<"  "<<(*PrSk[0])[2]<<"  "<<(*PrSk[0])[3]<<"  "<<(*PrSk[0])[4]<< std::endl;
			std::cout << "Covariance matrix elements: " << (*CvSk[0])[0][0] << "  " << (*CvSk[0])[0][3] << std::endl;
			VertexFit* Vskim = new VertexFit(1,PrSk, CvSk);
			Vskim->AddVtxConstraint(xpvc, covpvc);
			Double_t Chi2One = Vskim->GetVtxChi2();
			//Double_t Chi2One = 0.0;
