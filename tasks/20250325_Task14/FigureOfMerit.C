...
root -l EJ_reproduction/tasks/20250325_Task14/FigureOfMerit.C'(sel)' 
...
void FOM1D(Int_t sel,TFile* infile,const char *histname,Int_t samplenumber,Double_t firstcut,Double_t lastcut,Int_t ncuts,Double_t endcuts){
  ...
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250325_Task14/fom_%s_%d",histname,sel);
  c->SaveAs(outpdf + ".pdf");
  //c->Close();
...
void FOM2D_rectangle(Int_t sel,TFile* infile,const char *histname,Int_t samplenumber,Double_t firstcutx,Double_t lastcutx,Int_t ncutsx,Double_t firstcuty,Double_t lastcuty,Int_t ncutsy){
  ...
  // Save Figure Of Merit in a pdf
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250325_Task14/Rectangular_fom_%s_%d",histname,sel);
  c->SaveAs(outpdf + ".pdf");
  //c->Close();
...
void FOM2D_line(Int_t sel,TFile* infile,const char *histname,Int_t samplenumber,Double_t firstcutm,Double_t lastcutm,Int_t ncutsm,Double_t firstcutq,Double_t lastcutq,Int_t ncutsq){
  ...
  // Save Figure Of Merit in a pdf
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250325_Task14/2D_Linear_fom_%s_%d",histname,sel);
  c->SaveAs(outpdf + ".pdf");
  //c->Close();
