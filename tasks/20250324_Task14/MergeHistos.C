...
root -l EJ_reproduction/tasks/20250324_Task14/MergeHistos.C'(sel)' 
...
void DrawHistos(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,std::vector<Color_t> color,TString XTitle,TString YTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250324_Task14/%s_%d.pdf",outname,sel);
  c->SaveAs(outpdf);
...
void MergeHistos(Int_t sel=0)
{
  ...
  TString outname;
  outname.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250324_Task14/histos_sel%d.root",sel);
