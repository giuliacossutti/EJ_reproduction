...
root -l EJ_reproduction/tasks/20250906_ThesisPlots/MergeHistos.C'(sel)' 
...
void DrawHistos(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,std::vector<Color_t> color,TString XTitle,TString YTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  // Titles of axes
  h_stack->GetXaxis()->SetTitleSize(.050);
  h_stack->GetYaxis()->SetTitleSize(.050);
  h_stack->GetXaxis()->SetLabelSize(.050);
  h_stack->GetYaxis()->SetLabelSize(.050);
  ...
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/Zprimethesisplots/%s_%d.pdf",outname,sel);
  c->SaveAs(outpdf);
...
void DrawHistosStack(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,std::vector<Color_t> color,TString XTitle,TString YTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  TString outpdf;
  outpdf.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/Zprimethesisplots/%s_%d_stack.pdf",outname,sel);
  c->SaveAs(outpdf);
...
void DrawHistos2D(Int_t sel,std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,TString XTitle,TString YTitle,TString ZTitle,std::array<Double_t,4> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  for(Int_t i = 0; i < hist.size(); ++i){
    ...
    // Titles of axes
    hist.at(i)->GetXaxis()->SetTitleSize(.050);
    hist.at(i)->GetYaxis()->SetTitleSize(.050);
    hist.at(i)->GetZaxis()->SetTitleSize(.050);
    hist.at(i)->GetXaxis()->SetLabelSize(.040);
    hist.at(i)->GetYaxis()->SetLabelSize(.040);
    hist.at(i)->GetZaxis()->SetLabelSize(.040);
    ...
    outpdf.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/Zprimethesisplots/%s_sample%d_sel%d.pdf",outname,i,sel);
    c->SaveAs(outpdf);
...
void MergeHistos(Int_t sel=0)
{
  ...
  TString outname;
  outname.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/Zprimethesisplots/histos_sel%d.root",sel);
...
*/
  ...
  std::array<Double_t,4> leglimitsPT1_IM = {0.344612,0.661458,0.655388,0.885417};
  ...
  std::array<Double_t,4> leglimitsPT1_NC = {0.344612,0.661458,0.655388,0.885417};
