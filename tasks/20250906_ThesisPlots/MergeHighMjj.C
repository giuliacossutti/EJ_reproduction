...
root -l EJ_reproduction/tasks/20250906_ThesisPlots/MergeHighMjj.C 
*/
...
void DrawHistos2D(std::vector<TFile*> file,const char *histname,std::vector<Double_t> weights,Double_t lumi,TString XTitle,TString YTitle,TString ZTitle,std::vector<std::array<Double_t,4>> leglimits,TString legtitle,std::vector<TString> legentry,const char *outname){
  ...
  for(Int_t i = 0; i < hist.size(); ++i){
    ...
    hist.at(i)->GetXaxis()->SetLabelSize(.050);
    hist.at(i)->GetYaxis()->SetLabelSize(.050);
    hist.at(i)->GetZaxis()->SetLabelSize(.050);
    ...
    A.SetTextColor(kRed);
    A.SetTextFont(62);
    A.DrawLatex(0.15,0.15,"A");
    A.DrawLatex(0.15,0.22,"B");
    A.DrawLatex(0.22,0.15,"C");
    A.DrawLatex(0.22,0.22,"D");

    outpdf.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/Zprimethesisplots/%s_sample%d.pdf",outname,i);
    c->SaveAs(outpdf);
...
void MergeHighMjj()
{
  ...
  // Import histograms 
  TString signame;
  signame.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250428_Task17/signal.root");

  TString bkgTname;
  bkgTname.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250428_Task17/bkgTop.root");

  TString bkgHname;
  bkgHname.Form("/gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes/EJ_reproduction/tasks/20250428_Task17/bkgHardQCD.root");
  ...
  Double_t ntot_bkgT = 500000;
  Double_t ntot_bkgH = 500000;
  ...
  std::array<Double_t,4> leglimits_sig = {0.314536,0.651042,0.839599,0.890625};
  //std::array<Double_t,4> leglimits_bkg = {0.392231,0.15625,0.83584,0.435764};
  std::array<Double_t,4> leglimits_bkg = {0.333333,0.130208,0.83208,0.402778};

  std::vector< std::array<Double_t,4> > leglimits = {leglimits_sig,leglimits_bkg,leglimits_bkg};

   // write all scaled histograms on file
  TString outname;
  outname.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/Zprimethesisplots/histos_highmjj.root");
